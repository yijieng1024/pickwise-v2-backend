...
OPENROUTER_API_KEY=sk-or-v1-...             # agent LLM (app/agent/graph.py) + embeddings (app/embeddings/service.py) — served via OpenRouter
...
`GEMINI_API_KEY` is a required setting (`app/config.py`) and is passed explicitly (`google_api_key=settings.gemini_api_key`) to every Gemini client — processor, recommendation, review processor, and the eval judge. Do **not** rely on the ambient `GOOGLE_API_KEY` / Application Default Credentials — that won't exist in Docker/production.

`OPENROUTER_API_KEY` is also required: the conversational agent (`app/agent/graph.py`, `nvidia/nemotron-3-ultra-550b-a55b:free`) and all embeddings (`app/embeddings/service.py`, `nvidia/llama-nemotron-embed-vl-1b-v2:free`, `dimensions=768`) run through OpenRouter's OpenAI-compatible API. `:free` model variants have a small daily request cap unless the account holds ≥ $10 in credits.
...
AI Processor (Gemma gemma-4-31b-it) → laptops (normalized, multi-variant)
     ↓
Embeddings (nvidia/llama-nemotron-embed-vl-1b-v2 via OpenRouter, 768-dim) → laptop_embeddings
```
...
- **`app/embeddings/`** — `service.py` builds a natural-language document per laptop and embeds it via `nvidia/llama-nemotron-embed-vl-1b-v2:free` on OpenRouter's OpenAI-compatible `/embeddings` endpoint (`dimensions=768` — the model is natively 2048-dim but Matryoshka-trained, so truncation matches the `Vector(768)` column; `check_embedding_ctx_length=False` is required or langchain-openai sends tiktoken token arrays OpenRouter can't decode). `embed_text()` is the single embedding entry point for the whole app (hybrid search, RAG retrieval, review chunks, recommendation). Changing the embedding model changes the vector space — always re-run `POST /embeddings/generate-all` (and re-process any review chunks) afterwards. `router.py` exposes admin-only generate-all / generate-single / status endpoints
