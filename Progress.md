...
- **Email**: Brevo HTTP API (`api.brevo.com/v3/smtp/email`, via `httpx`) with background tasks — HTTP because Render blocks outbound SMTP ports
...
- **Google Sign-In** (`POST /auth/google`): frontend obtains an ID token via Google Identity Services and posts `{id_token}`; backend verifies signature/expiry/audience with `google-auth` against `GOOGLE_OAUTH_CLIENT_ID` (optional setting — endpoint 400s if unset). Resolution order: returning user by `provider_sub` (Google's stable `sub`) → existing local account by email (linked + marked verified, keeps its password) → new account (unique username generated from email prefix, `password=None`, `auth_provider="google"`, `is_verified=True` — no SMTP verification needed). Returns the same `Token` as `/auth/login`. Google-only users (`password IS NULL`) no longer gain a password via forgot-password — that flow now mails them a "use Continue with Google" notice instead
